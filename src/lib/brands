...
//
//  현재 사내에 브랜드 목록이 두 갈래로 존재합니다.
//   · 예산효율/세부항목 탭 : 꿈비, 파미야, 뉴어스, 소브, 오가닉그라운드, 바바디토, G7커피
//   · KPI1_/경쟁사트렌드 탭 : 꿈비리코코, 봄봄슈슈비, 꿈비육아, 오가닉그라운드, 바바디토, 파미야
//
//  여기에 사업팀 매출이 세 번째 축으로 들어오면 연결이 불가능해지므로,
//  이 파일을 "단 하나의 기준"으로 삼고 나머지는 전부 여기로 번역해서 씁니다.
//
//  ▶ 구조
//   · BRAND (브랜드)  : 매출·예산이 집계되는 단위. 사업팀과 맞추는 축.
//   · LINE  (제품라인) : 트렌드·경쟁사 분석이 이뤄지는 단위. 브랜드보다 잘게 쪼개짐.
...
export const LINE_TO_BRAND: Record<string, Brand> = {
  ...
  봄봄슈슈비: "꿈비", // ⚠️ 확인 필요 — 별도 브랜드로 분리할지 결정 대상

  // 브랜드명과 라인명이 같은 경우 (1:1)
  오가닉그라운드: "오가닉그라운드",
  바바디토: "바바디토",
  파미야: "파미야",
};
